...
* 当検証では最大文書数500件に対して、廉価な`gpt-3.5-turbo`を用いたインデックス化を試み、以下の検証結果を得た．
  ...
  - 上記より、対話応答の精度は`gpt-3.5-turbo`では不十分

* 検証結果より、運用という文脈ではインデックス化は廉価な`gpt-3.5-turbo`を使い、対話応答は安定的な日本語回答可能な`gpt-4`を使用するのが良いと思料．
* 今後の検証の方向性

  全文書（`13698`件）に対して`gpt-3.5-turbo`を使ってインデックス化し、応答時間を計測し、運用に耐えられるものかを試す．
